...
with open(outputFile, 'w',newline='', encoding='utf-8') as csvfile:
    ...
    for raw in jsonData:
        ...
        rows.append(["{}{}".format(raw["DataValidade"][:-2],int(raw["DataValidade"][-2:])-2),raw["DataValidade"], dictOperador[raw["Operador"]], "PJ" if raw["CPFCNPJ"].find("/") != -1 else "PF", raw["TipoUso"], raw["Fabricante"].split(" ")[0].upper().replace("Ã","A").replace("Õ","O").replace("Ó","O").replace("Ç","C").replace("Á","A").replace("Í","I"), raw["Modelo"], raw["Numerodeserie"], raw["Pesomaximodedecolagem"], raw["CidadeEstado"][:-3], raw["CidadeEstado"][-2:], raw["Ramodeatividade"] if raw["Ramodeatividade"] != None else "Não Informado"])
